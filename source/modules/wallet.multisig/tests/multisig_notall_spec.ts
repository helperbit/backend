...
import {
	...
	geolocalizeAsAffected, addAdmins
} from '../../tests.shared/middlewares/user';
import {
	createMultisig, feedMultisig, balance,
	faucet, withdrawFee, getWallet, signMultisigNotAll
} from '../../tests.shared/middlewares/wallet';
import { frisbyChain } from '../../tests.shared/middlewares/frisby-chain';


/* Create admins */
frisbyChain({ username: 'testmultisignotall_ms1' }, [
	common.cleanResources('testmultisignotall'),
	createAdmins('testmultisignotall'),

	(data, next) => next({
		username: 'testmultisignotall_msmaster',
		usertype: 'npo',
		admins: data.admins,
