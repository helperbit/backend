...
import {
	signup, login, checkToken, acceptVerification,
	setAllowedAdminsVerify
...
const chain = new TestChain('testadminautoafteradd');

chain.pushData({ username: 'testadminautoafteradd_npo', usertype: 'npo' });
chain.push(signup);
chain.push(login);
	...
	admins: [
		{
			firstname: "Gianni",
			lastname: "Pinolo",
			email: "testadminautoafteradd_ms1@gmail.com",
			idnumber: "AX4535435"
		},
		{
			firstname: "Gianni",
			lastname: "Pinolo",
			email: "testadminautoafteradd_ms2@gmail.com",
			idnumber: "AX4535435"
		},
		{
			firstname: "Gianni",
			lastname: "Pinolo",
			email: "testadminautoafteradd_ms3@gmail.com",
			idnumber: "AX4535435"
		}
	]
}).authenticate().expect(200));
...
chain.pushData(data => ({ username: 'testadminautoafteradd_ms1', npo: data }));
chain.push(signup);
chain.push(login);
chain.push(checkToken);
chain.pushReq(TestRequest.get('/me', 'check adminof')
	.authenticate()
	.expect(200, { adminof: ["testadminautoafteradd_npo"] }));

chain.pushData(data => ({ username: 'testadminautoafteradd_ms2', npo: data.npo }));
chain.push(signup);
chain.push(login);
chain.push(checkToken);
chain.pushReq(TestRequest.get('/me', 'check adminof')
...
chain.pushData(data => ({ username: 'testadminautoafteradd_ms3', npo: data.npo }));
chain.push(signup);
chain.push(login);
chain.push(checkToken);
chain.pushReq(TestRequest.get('/me', 'check adminof')
...
chain.pushData(data => ({ username: 'testadminautoafteradd_npo', password: 'testadminautoafteradd_npo' }));
chain.push(login);
chain.push(checkToken);
chain.pushReq(TestRequest.post('/me', 'complete profile', {
...
chain.pushReq(TestRequest.get('/me/admin', 'check admins')
	.authenticate()
	.expect(200, {
		admins: ["testadminautoafteradd_ms1@gmail.com", "testadminautoafteradd_ms2@gmail.com", "testadminautoafteradd_ms3@gmail.com"]
	}));
