...
export default class SocialFacebookFollowersMetric implements BackofficeMetric {
	code = "social-facebook-followers";
	ui = {
		name: "Social Facebook Followers",
		description: "Social followers on Facebook",
		icon: 'fa-facebook',
		color: '#3b5998',
		enabled: false,
		category: 'Social'
	...
	async total(): Promise<number> {
		const res = await AdminMetrics.aggregate().match({}).group({ _id: 1, value: { $max: "$social.facebook" } }).exec();
		if (res.length > 0)
			return res[0]['value'];
		return 0;
	}
	...
	async chart(timeframe: 'day' | 'week' | 'month' | 'year', start: Date, end: Date): Promise<QueryHelper.ChartData> {
		return await QueryHelper.chart(AdminMetrics, {
			...
			timeframe: timeframe,
			aggregator: { $max: '$social.facebook' },
