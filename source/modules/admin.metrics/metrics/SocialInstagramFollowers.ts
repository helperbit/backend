...
export default class SocialInstagramFollowersMetric implements BackofficeMetric {
	code = "social-instagram-followers";
	ui = {
		name: "Social Instagram Followers",
		description: "Social followers on Instagram",
		icon: 'fa-instagram',
		color: '#8a3ab9',
		enabled: false,
		category: 'Social'
	...
	async total(): Promise<number> {
		const res = await AdminMetrics.aggregate().match({}).group({ _id: 1, value: { $max: "$social.instagram" } }).exec();
		if (res.length > 0)
			return res[0]['value'];
		return 0;
	}
	...
	async chart(timeframe: 'day' | 'week' | 'month' | 'year', start: Date, end: Date): Promise<QueryHelper.ChartData> {
		return await QueryHelper.chart(AdminMetrics, {
			...
			timeframe: timeframe,
			aggregator: { $max: '$social.instagram' },
