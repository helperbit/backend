...
export default class FinancialBurnRateMetric implements BackofficeMetric {
	code = "financial-burn-rate";
	ui = {
		name: "Burn rate (Euro)",
	...
	async total(): Promise<number> {
		const res = await AdminMetrics.aggregate().match({}).group({ _id: 1, value: { $sum: "$burn_rate" } }).exec();
		if (res.length > 0)
			return res[0]['value'];
		return 0;
	}
