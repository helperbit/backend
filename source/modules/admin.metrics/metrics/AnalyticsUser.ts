...
export default class AnalyticsUserMetric implements BackofficeMetric {
	code = "analytics-user";
	ui = {
		name: "Analytics User",
	...
	async total(): Promise<number> {
		const res = await AdminMetrics.aggregate().match({}).group({ _id: 1, value: { $sum: "$analytics.users" } }).exec();
		if (res.length > 0)
			return res[0]['value'];
		return 0;
	}
	...
	async chart(timeframe: 'day' | 'week' | 'month' | 'year', start: Date, end: Date): Promise<QueryHelper.ChartData> {
		return await QueryHelper.chart(AdminMetrics, {
			...
			timeframe: timeframe,
			aggregator: { $sum: '$analytics.users' },
			field: 'date'
		});
	}
}
