...
export default class FinancialProfitMetric implements BackofficeMetric {
	code = "financial-profit";
	ui = {
		name: "Profit (Euro)",
		description: "Profit",
		icon: 'fa-eur',
	...
	async total(): Promise<number> {
		const res = await AdminMetrics.aggregate().match({}).group({ _id: 1, value: { $sum: "$profit" } }).exec();
		if (res.length > 0)
			return res[0]['value'];
		return 0;
	}
